...
if(x_count("userdb_bank","id='$userid' AND status='1' LIMIT 1") > 0){
			...
			x_insert("userid,amount,bankname,acctname,transfer_date,date_time,timereal,type,status,token,os,br,ip","alertus","'$userid','$mainamt','$bn','$accountname','$tdate','$time','$rtime','topup','0','$token','$os','$br','$ip'","<p class='hubmsg'>Alert of <b>($wc $mamt)</b> sent successfully! Your wallet will be credited after payment confirmation. Please do not resend this alert. </p>","<p class='hubmsg'>Failed to send alert</p>");
