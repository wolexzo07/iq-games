...
?>
<div align="center">
<h1 style="color:;">WORDS <font class="colorg">SCRAMBLER</font> GAME</h1>
<?php
...
<input maxlength="8" required type="text" placeholder="Enter the word to scramble (8 letter)" class="we" name="scramble"/>
<p class="let">Predict the first scrambled word (8 letter)</p>
<input maxlength="8" required type="text" placeholder="Predict the first scrambled word (8 letter)" class="we" name="first"/>
<p class="let">Predict the second scrambled word (8 letter)</p>
<input maxlength="8" required type="text" class="we" placeholder="Predict the first scrambled word (8 letter)" name="second"/><br/>
...
if(isset($_POST["scramble"]) && !empty($_POST["scramble"])){
	...
	if((strlen($word) < 8) || (strlen($first) < 8) || (strlen($second) < 8)){
		...
		if(in_array($final,$array)){
			...
			<caption class="dec">Congratulations! You prediction was correct.</caption>
			<tr>
			...
			?><table cellpadding="10px" cellspacing="0px" border="1px">
			<caption class="dec">Failed! Try again</caption>
			<tr>
			<td>Word scrambled</td>
			<th><?php echo $word;?></th>
			</tr>
