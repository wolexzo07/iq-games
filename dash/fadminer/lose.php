...
$query="select id from games_played WHERE status='lost' OR status='' order by id desc";
}
...
$query="SELECT * FROM games_played WHERE status='lost' OR status='' ORDER BY id desc
limit ".mysqli_real_escape_string(x_cstring(),$start).",$recordsPerPage";
}
...
<tr <?php
if($status == "won"){
?>style="color:white;background-color:gray;"<?php
}
?>>
