...
$query="select id from questionbank WHERE status='1' order by id desc";
}
...
if(isset($_GET['call']) && !empty($_GET['call'])){
$call = xclean($_GET['call']);
$query="SELECT * FROM questionbank WHERE questionbank LIKE '%$call%' ORDER BY id desc
limit ".mysqli_real_escape_string(x_cstring(),$start).",$recordsPerPage";
}else{
$query="SELECT * FROM questionbank WHERE status='1' ORDER BY id desc
limit ".mysqli_real_escape_string(x_cstring(),$start).",$recordsPerPage";
}
...
else
{
	...
	$msg = "<p class='text-center' style='font-size:60pt;margin-bottom:10pt;'><span class='glyphicon glyphicon-check'></span></p>";
$msg .= "<p class='text-center'>No question was approved!!</p>";
			echo $msg;
